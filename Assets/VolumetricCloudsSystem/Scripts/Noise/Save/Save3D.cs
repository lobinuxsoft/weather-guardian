...
public class Save3D : MonoBehaviour {
    ...
    public void Save (RenderTexture volumeTexture, string saveName) {
        ...
        for (int layer = 0; layer < resolution; layer++) {
            RenderTexture slice = new RenderTexture (resolution, resolution, 0);
        ...
        Texture3D tex3D = Tex3DFromTex2DArray (slices, resolution);

        string pathToSave = UnityEditor.EditorUtility.SaveFilePanelInProject("Save 3D Noise", saveName, "asset", "Save the 3D noise texture on Resources folder...");

        if (!string.IsNullOrEmpty(pathToSave))
            UnityEditor.AssetDatabase.CreateAsset (tex3D, pathToSave);
#endif
    }
